module game (
    ...
    localparam DOWN = 0, UP = 1, RIGHT = 2, LEFT = 3;
    ...
    reg [31:0] tick; // modulo 2^32

    always @(posedge clk) begin
        if (~reset_n) tick <= 0;
        else if (P != S_MAIN_INIT) tick <= tick + 1;
    end
    ...
    always @(posedge clk) begin
        if (~reset_n) begin
            ...
            sram_addr1 <= {tick[1:0], map_pos_next[1], map_pos_next[0]};
        end else begin
            init_counter <= 0;
        end
    end
        ...
        if (~reset_n) begin
            ...
            if (snake_counter == snake_rate) begin
                ...
                if (map[next_head_pos[0]][next_head_pos[1]] == 0) begin
                    map[next_head_pos[0]][next_head_pos[1]] <= {4'h4, next_heading};
                    map[head_pos[0]][head_pos[1]] <= {2'h0, map[head_pos[0]][head_pos[1]][1:0] ^ 2'h1, next_heading};
                    map[next_tail_pos[0]][next_tail_pos[1]] <= {4'h5, map[next_tail_pos[0]][next_tail_pos[1]][1:0]};
                ...
                if (food_gone && map[map_pos[0 +: 4]][map_pos[4 +: 4]] == 0) begin
