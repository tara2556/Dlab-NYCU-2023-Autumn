...
localparam [3:0] S_MAIN_INIT = 0, S_MAIN_PROMPT1 = 1,
                 S_MAIN_READ_NUM1 = 2,S_MAIN_PROMPT2 = 3,
                 S_MAIN_READ_NUM2 = 4,
                 S_MAIN_DIVD = 5, S_MAIN_REPLY = 6;
localparam [1:0] S_UART_IDLE = 0, S_UART_WAIT = 1,
                 S_UART_SEND = 2, S_UART_INCR = 3;
localparam INIT_DELAY = 100_000; // 1 msec @ 100 MHz
localparam PROMPT1_STR = 0;  // starting index of the prompt message
localparam PROMPT1_LEN = 23; // length of the prompt message
localparam PROMPT2_STR = 23;  // starting index of the prompt message
localparam PROMPT2_LEN = 24; // length of the prompt message
localparam ANS_LEN = 28;
localparam ANS_STR = 47;
localparam MEM_SIZE   = 75;
...
reg  [0:PROMPT1_LEN*8-1] msg1 = { "\015\012Enter first number: ", 8'h00 };
reg  [0:PROMPT2_LEN*8-1] msg2 = { "\015\012Enter second number: ", 8'h00 };
reg  [0:ANS_LEN*8-1] msg3 = {"\015\012The answer was: 0x0000.\015\012", 8'h00};
reg  [15:0] num1_reg;  // The key-in number register
reg  [15:0] num2_reg;  // The key-in number register
reg  [15:0] ans;
...
always @(posedge clk) begin
  if (~reset_n) begin
    ...
    for (idx = 0; idx < ANS_LEN; idx = idx + 1) data[idx+ANS_STR] = msg3[idx*8 +: 8];
    
  end
  else if (P == S_MAIN_REPLY) begin
        data[ANS_STR+20] <= ((ans[15:12] > 9)? "7" : "0") + ans[15:12];
        data[ANS_STR+21] <= ((ans[11: 8] > 9)? "7" : "0") + ans[11: 8];
        data[ANS_STR+22] <= ((ans[ 7: 4] > 9)? "7" : "0") + ans[ 7: 4];
        data[ANS_STR+23] <= ((ans[ 3: 0] > 9)? "7" : "0") + ans[ 3: 0];
...
always @(*) begin // FSM next-state logic
  case (P)
    ...
    S_MAIN_READ_NUM2: // wait for <Enter> key.
      if (enter_pressed) P_next = S_MAIN_DIVD;
      else P_next = S_MAIN_READ_NUM2;
    S_MAIN_DIVD: // wait for division
      if (calc == 0) P_next = S_MAIN_REPLY;
      else P_next = S_MAIN_DIVD;
...
assign print_enable = (P == S_MAIN_INIT && P_next == S_MAIN_PROMPT1) ||
                      ...
                      (P == S_MAIN_DIVD   && P_next == S_MAIN_REPLY);
...
always @(posedge clk) begin
  case (P_next)
    ...
    S_MAIN_READ_NUM2: send_counter <= ANS_STR;
...
always @(posedge clk)begin
  ...
  ans <= 0;
    ...
    num2_reg <= 0;
       ...
       if (received && is_num_key) begin
            if(P==S_MAIN_READ_NUM1) begin num1_reg <= (num1_reg * 10) + (rx_byte - 48);  end
            else if(P==S_MAIN_READ_NUM2) begin num2_reg <= (num2_reg * 10)+(rx_byte-48); end
           
       end
        else if(P==S_MAIN_DIVD) begin 
                if ((num1_reg>>calc) >= num2_reg) begin
                ans <= ans | (1<<calc);
                num1_reg <= num1_reg - (num2_reg<<calc);
                end
                calc <= calc- 1;
